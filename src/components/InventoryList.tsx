...
import { Plus, Minus, Search, AlertCircle, Coffee } from 'lucide-react';
    ...
    <div className="p-6 max-w-md mx-auto min-h-screen pb-24">
      ...
      <header className="flex items-center justify-between mb-8">
        ...
        <button 
          ...
          className="flex items-center gap-1.5 px-4 py-2 bg-wood-light text-coffee-brown rounded-xl font-semibold text-sm hover:bg-wood-medium/40 transition-colors active:scale-95"
        >
      ...
      <div className="flex flex-col gap-4">
        {filteredProducts.map((product) => {
          ...
          return (
            ...
            >
              ...
              <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isLowStock ? 'bg-accent-red-border/50' : 'bg-wood-light'}`}>
                {/* Cantidad Actual */}
                <div className="flex items-baseline gap-1">
                  <span className={`text-2xl font-bold ${isLowStock ? 'text-accent-red' : 'text-coffee-dark'}`}>
                    {product.currentQuantity}
                  </span>
                  <span className={`font-medium ${isLowStock ? 'text-accent-red' : 'text-text-muted'}`}>{product.unit}</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    onClick={() => updateStock(product.id, 1)}
                    className="w-10 h-10 bg-coffee-brown text-white rounded-xl flex items-center justify-center shadow-[0_4px_8px_rgba(92,61,46,0.2)] transition-all active:scale-90"
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-10 text-text-muted font-medium bg-white rounded-2xl border border-wood-light border-dashed">
            ...
            <button 
              onClick={() => setIsModalOpen(true)}
              className="mt-4 px-6 py-2 bg-coffee-brown text-white rounded-xl font-medium mx-auto block"
            >
